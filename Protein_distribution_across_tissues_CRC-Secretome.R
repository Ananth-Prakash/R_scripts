...
######## colorectal cancer Secretome data - Javier ###########
...
input_data <- read.table(file = "proteinGroups_ppb_final-tissue_names.txt", quote = "\"", header = TRUE, sep = "\t", stringsAsFactors = FALSE, comment.char = "#")
...
Blood_derived <- input_data[grepl("GeneName|Blood.derived", colnames(input_data), ignore.case = TRUE)]
Cellculture_derived <-  input_data[grepl("GeneName|Cellculture.derived", colnames(input_data), ignore.case = TRUE)]
ECvesicles_and_exosomes <- input_data[grepl("GeneName|ECvesicles.and.exosomes", colnames(input_data), ignore.case = TRUE)]
Interstitial_fluid <- input_data[grepl("GeneName|Interstitial.fluid", colnames(input_data), ignore.case = TRUE)]

# Get sample sizes for each organ (i.e., number of MS runs representing each organ)
sample_sizes <- as.data.frame(ncol(Blood_derived)-1)
sample_sizes <- cbind(sample_sizes, ncol(Cellculture_derived)-1)
sample_sizes <- cbind(sample_sizes, ncol(ECvesicles_and_exosomes)-1)
sample_sizes <- cbind(sample_sizes, ncol(Interstitial_fluid)-1)
...
Blood_derived_iBAQ_long <- gather(Blood_derived, Sample, iBAQ, colnames(Blood_derived)[2]:colnames(Blood_derived)[ncol(Blood_derived)], factor_key=TRUE)
Cellculture_derived_iBAQ_long <- gather(Cellculture_derived, Sample, iBAQ, colnames(Cellculture_derived)[2]:colnames(Cellculture_derived)[ncol(Cellculture_derived)], factor_key=TRUE)
ECvesicles_and_exosomes_iBAQ_long <- gather(ECvesicles_and_exosomes, Sample, iBAQ, colnames(ECvesicles_and_exosomes)[2]:colnames(ECvesicles_and_exosomes)[ncol(ECvesicles_and_exosomes)], factor_key=TRUE)
Interstitial_fluid_iBAQ_long <- gather(Interstitial_fluid, Sample, iBAQ, colnames(Interstitial_fluid)[2]:colnames(Interstitial_fluid)[ncol(Interstitial_fluid)], factor_key=TRUE)


All_iBAQ_long <- rbind(Blood_derived_iBAQ_long,Cellculture_derived_iBAQ_long,ECvesicles_and_exosomes_iBAQ_long,
                       Interstitial_fluid_iBAQ_long)
...
Tissue_median <- as.data.frame(cbind(GeneName = Blood_derived$GeneName, Blood_derived = apply(as.data.frame(Blood_derived[,-c(1)]), 1, median, na.rm = T)))
Tissue_median <- merge(x=Tissue_median, y=cbind(GeneName = Cellculture_derived$GeneName, Cellculture_derived = apply(Cellculture_derived[,-c(1)], 1, median, na.rm = T)), by.x="GeneName", by.y="GeneName", all.x=TRUE, all.y=TRUE)
Tissue_median <- merge(x=Tissue_median, y=cbind(GeneName = ECvesicles_and_exosomes$GeneName, ECvesicles_and_exosomes = apply(as.data.frame(ECvesicles_and_exosomes[,-c(1)]), 1, median, na.rm = T)), by.x="GeneName", by.y="GeneName", all.x=TRUE, all.y=TRUE)
Tissue_median <- merge(x=Tissue_median, y=cbind(GeneName = Interstitial_fluid$GeneName, Interstitial_fluid = apply(as.data.frame(Interstitial_fluid[,-c(1)]), 1, median, na.rm = T)), by.x="GeneName", by.y="GeneName", all.x=TRUE, all.y=TRUE)

write.table(Tissue_median, file = paste("Gene_distribution_in_Tissues-GeneNames-Median_intensities-CRC_Secretome.txt", sep=""), sep = "\t", row.names = FALSE, quote = FALSE )
...
ggplot(plotdata[plotdata$Present_in_number_of_samples !=0,], aes(x=Present_in_number_of_samples, y=Gene_percent)) + 
  ...
  xlab("Present in number of secretome")+
...
write.table(gene_counts_per_tissue, file = paste("Gene_distribution_in_tissues_plot-CRC_Secretome.txt", sep=""), sep = "\t", row.names = FALSE, quote = FALSE )
...
Datasets_median <- as.data.frame(cbind(GeneName = All_datasets$GeneName, PXD020454_median = apply(as.data.frame(All_datasets[,grep("PXD020454", colnames(All_datasets))]), 1, median, na.rm = T)))
Datasets_median <- merge(x=Datasets_median, y=cbind(GeneName = All_datasets$GeneName, PXD010458_median = apply(as.data.frame(All_datasets[,grep("PXD010458", colnames(All_datasets))]), 1, median, na.rm = T)), by.x="GeneName", by.y="GeneName", all.x=TRUE, all.y=TRUE)
Datasets_median <- merge(x=Datasets_median, y=cbind(GeneName = All_datasets$GeneName, JPST000867_median = apply(as.data.frame(All_datasets[,grep("JPST000867", colnames(All_datasets))]), 1, median, na.rm = T)), by.x="GeneName", by.y="GeneName", all.x=TRUE, all.y=TRUE)
Datasets_median <- merge(x=Datasets_median, y=cbind(GeneName = All_datasets$GeneName, PXD005693_median = apply(as.data.frame(All_datasets[,grep("PXD005693", colnames(All_datasets))]), 1, median, na.rm = T)), by.x="GeneName", by.y="GeneName", all.x=TRUE, all.y=TRUE)
Datasets_median <- merge(x=Datasets_median, y=cbind(GeneName = All_datasets$GeneName, PXD031556_median = apply(as.data.frame(All_datasets[,grep("PXD031556", colnames(All_datasets))]), 1, median, na.rm = T)), by.x="GeneName", by.y="GeneName", all.x=TRUE, all.y=TRUE)
Datasets_median <- merge(x=Datasets_median, y=cbind(GeneName = All_datasets$GeneName, PXD005709_median = apply(as.data.frame(All_datasets[,grep("PXD005709", colnames(All_datasets))]), 1, median, na.rm = T)), by.x="GeneName", by.y="GeneName", all.x=TRUE, all.y=TRUE)
Datasets_median <- merge(x=Datasets_median, y=cbind(GeneName = All_datasets$GeneName, PXD032899_median = apply(as.data.frame(All_datasets[,grep("PXD032899", colnames(All_datasets))]), 1, median, na.rm = T)), by.x="GeneName", by.y="GeneName", all.x=TRUE, all.y=TRUE)
...
dataset_tissues <- data.frame(Datasets= "PXD020454", Tissue_count = length(unique(gsub(".*\\.", "", dataset_sample_names[grep("PXD020454", dataset_sample_names)], perl=TRUE))) )
dataset_tissues <- rbind(dataset_tissues, data.frame(Datasets= "PXD010458", Tissue_count = length(unique(gsub(".*\\.", "", dataset_sample_names[grep("PXD010458", dataset_sample_names)], perl=TRUE))) ))
dataset_tissues <- rbind(dataset_tissues, data.frame(Datasets= "JPST000867", Tissue_count = length(unique(gsub(".*\\.", "", dataset_sample_names[grep("JPST000867", dataset_sample_names)], perl=TRUE))) ))
dataset_tissues <- rbind(dataset_tissues, data.frame(Datasets= "PXD005693", Tissue_count = length(unique(gsub(".*\\.", "", dataset_sample_names[grep("PXD005693", dataset_sample_names)], perl=TRUE))) ))
dataset_tissues <- rbind(dataset_tissues, data.frame(Datasets= "PXD031556", Tissue_count = length(unique(gsub(".*\\.", "", dataset_sample_names[grep("PXD031556", dataset_sample_names)], perl=TRUE))) ))
dataset_tissues <- rbind(dataset_tissues, data.frame(Datasets= "PXD005709", Tissue_count = length(unique(gsub(".*\\.", "", dataset_sample_names[grep("PXD005709", dataset_sample_names)], perl=TRUE))) ))
dataset_tissues <- rbind(dataset_tissues, data.frame(Datasets= "PXD032899", Tissue_count = length(unique(gsub(".*\\.", "", dataset_sample_names[grep("PXD032899", dataset_sample_names)], perl=TRUE))) ))
...
write.table(gene_counts_per_dataset, file = paste("Gene_distribution_in_datasets_plot-CRC_Secretome.txt", sep=""), sep = "\t", row.names = FALSE, quote = FALSE )
...
ggplot(gene_counts_per_dataset, aes(x=Datasets_tissues, y=Number_of_identified_genes)) + 
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+
  ggtitle("Distribution of canonical proteins across\n datasets (CRC-Secretome)")+
  theme(axis.text=element_text(size=12),
        axis.title=element_text(size=12))
