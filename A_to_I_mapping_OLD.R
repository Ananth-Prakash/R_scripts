...
all_editing_sites <- read.table(file = "filteredTable.txt", quote = "\"", header = TRUE, sep = "\t", stringsAsFactors = FALSE, fill =TRUE) 
...
ggplot(data = filtered_editing_sites, aes(x = nSamples)) +
  ...
  geom_vline(aes(xintercept = median_nSamples), color = "red", linewidth = 1)+
  scale_x_log10()+
  theme_bw()+
...
ggplot(data = synfiltered_editsite_distribution , aes(x = n)) +
  ...
  #scale_x_log10()+
  scale_y_log10()+
  xlab("Number of AtoI edit sites in each peptide fragment")+
...
# Limit peptide fragment length to between 7 and 70 amino acids
...
#Number_of_peptide_fragment_variations <- read.table(file = "Plots/ForPlot4-Number_of_peptide_fragment_variations.txt", quote = "\"", header = TRUE, sep = "\t", stringsAsFactors = FALSE, fill =TRUE) 
